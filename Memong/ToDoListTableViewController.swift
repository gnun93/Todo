...
class ToDoListTableViewController: UITableViewController {
    ...
    @IBAction func switchValueChange(_ sender: UISwitch) {
        ToDo.dummyToDoList[sender.tag - 100].done = sender.isOn
    }
    
    
    @IBOutlet weak var editButton: UIBarButtonItem!
    
    @IBAction func editDeleteButtonTouchUpInside(_ sender: Any) {
        ...
        if (self.tableView.isEditing) {
            ...
            navigationItem.leftBarButtonItem?.title = "확인"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.subject.text = target.content
        cell.dateLabel?.text = formatter.string(from: target.insertDate)
        cell.done.isOn = target.done
        
        cell.done.tag = indexPath.row + 100
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            _ = ToDo.dummyToDoList[indexPath.row]
            ToDo.dummyToDoList.remove(at: indexPath.row)
